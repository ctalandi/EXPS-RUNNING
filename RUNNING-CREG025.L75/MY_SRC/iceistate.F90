...
!CT for SEDNA { in case of CREG025.L75 we read leads fraction and heree we want ice concentration  = 1. - frld ice leads
               ! the input file CREG025_SOSIE-CREG12_drowned_ORCA12.L46-MAL95_y1998-2007m01_icemod.nc is concerned
               !zat_i_ini(:,:) = si(jp_ati)%fnow(:,:,1) * tmask(:,:,1)
               zat_i_ini(:,:) = ( 1._wp - si(jp_ati)%fnow(:,:,1) ) * tmask(:,:,1)
!CT for SEDNA }
